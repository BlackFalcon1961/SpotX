...
function downloadSp() {
    ...
    $web_Url = "https://upgrade.scdn.co/upgrade/client/win32-x86/spotify_installer$online.exe"
    ...
    try { 
        if ($curl_check) {
            $stcode = curl.exe -s -w "%{https_code}" -o /dev/null $web_Url --retry 2 --ssl-no-revoke
            if ($stcode -ne "200") {
